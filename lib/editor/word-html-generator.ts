// lib/editor/word-html-generator.ts
...
export const ALIGNMENT_KEY = 'sf_editor_alignments';

export function processTextHtml(html: string): string {
  if (!html) return '';
...
export function getImageDimensions(base64Data: string, mimeType: string): { width: number; height: number } | null {
  try {
...
export async function getBase64FromUrl(url: string): Promise<{ mimeType: string; base64Data: string } | null> {
  if (!url) return null;
...
export const getHtmlHeader = (title: string) => `
    ...
    switch (block.type) {
      ...
      case 'table': {
        ...
        if (block.data.content) {
          block.data.content.forEach((row, rIndex) => {
            bodyContent += '<tr>';
            row.forEach((cell) => {
