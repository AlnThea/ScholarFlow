      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-2xl w-full max-w-md flex flex-col gap-5 animate-scale-in text-slate-800 dark:bg-slate-900 dark:border-slate-700/80 dark:text-slate-200">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          <h3 className="text-sm font-extrabold text-slate-800 dark:text-white">
            {isEditing 
              ? (isEn ? 'Edit Math Formula (LaTeX)' : 'Edit Rumus Matematika (LaTeX)')
              : (isEn ? 'Insert Math Formula (LaTeX)' : 'Sisipkan Rumus Matematika (LaTeX)')}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-col gap-3">
          <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
            {isEn 
              ? <>Enter LaTeX code for the math formula you want to insert (e.g., <code>{`\\frac{a}{b}`}</code> or <code>{`\\sum x^2`}</code>).</>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">{isEn ? 'LaTeX Formula' : 'Formula LaTeX'}</label>
            <input
              type="text"
              placeholder="E = mc^2"
              value={mathFormulaInput}
              onChange={(e) => setMathFormulaInput(e.target.value)}
              className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg text-xs font-mono focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition"
              autoFocus
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  onConfirm();
        ...
        <div className="flex justify-end gap-2.5 pt-3 border-t border-slate-100 dark:border-slate-800">
          ...
          >
            {isEditing 
              ...
              : (isEn ? 'Insert Formula' : 'Sisipkan Rumus')}
          </button>
        </div>
      </div>
    </div>,
